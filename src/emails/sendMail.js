...
const sendCancellationEmail = to => {
  const mailOptions = {
    ...
    generateTextFromHTML: true,
    html: `<div>We’re sorry to see you go but don’t worry</div>
    <div>you can come back at anytime to ensure you don’t miss out on everything ResumeCoach has to offer.</div>
    <div>Your subscription has been cancelled.</div>
    <div>Thanks again for trying Cleaning App!</div>
    <div>Here’s to wishing you a long, fruitful career.</div>
    <div>Ksenia,</div> 
    <div>Cleaning App</div>
    </div><br>`
  };
  sendMail(mailOptions);
};
